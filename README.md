...
跑完頁面上會有一個「開回報表單」的按鈕，直接帶你到
[裝置回報表單](../../issues/new?template=device-report.yml)。

**不管結果是好是壞、甚至跑失敗，都有用。** 失敗本身就是資料 ——
前幾份回報就是這樣抓出四個 bug 的，其中兩個會產生看起來很合理、但其實是錯的數字。
...
**其他的回報管道**：東西壞了或文件寫錯了，開一個
[錯誤回報](../../issues/new?template=bug-report.yml)；
只是想問問題、或想聊聊架構為什麼這樣設計，去
[Discussions](../../discussions) 會比開 issue 輕鬆。
